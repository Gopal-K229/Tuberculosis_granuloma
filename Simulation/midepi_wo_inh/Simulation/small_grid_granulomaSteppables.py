...
surface_coeff = input("enter the surface coeff:")
...
surface_coeff = float(surface_coeff)
...
print("simulation started with surface coeff:", surface_coeff)
...
class mac_recruitment_Steppable(SteppableBasePy):
	...
	def step(self, mcs):
		...
		if 250 < mcs and random.random() < 0.1:
			...
			# if mcs<10000:
				cell.targetSurface = surface_coeff
	...
	def step(self, mcs):
		...
		if mcs %50 ==0:
			for cell in self.cell_list:
				...
				cell_data = [cell.id, type_map.get(cell.type, "unknown"), cell.xCOM, cell.yCOM, attr_concentration, oxy_concentration, mcs] #inh_concentration
				df1.append(cell_data)
			dataframe1 = pd.DataFrame(df1, columns=['cell_id', 'cell_type', 'cell_xcoordinate','cell_ycoordinate', 'attr_concentration', 'oxy_concentration','mcs']) # 'inh_concentration', 
